...
validator_address="B62qmHktvgpH1Kbkato9MCWmH4TPKtHWc8DC4FhQCJ5TpuZ8CXazczB"
TG_TOKEN=$1
CHAT_ID=$2
...
while true; do
  ...
  if [[ $previous_place >  $new_place ]]; then
    MSG="We moved up $(echo ${previous_place}-${new_place} | bc) position(s). Current position is ${new_place}"
    previous_place=${new_place}
    send_message ${MSG}
    
  elif [[ $previous_place <  $new_place ]]; then
    MSG="We have lost $(echo ${new_place}-${previous_place} | bc) position(s). Current position ${new_place}"
    previous_place=${new_place}
    send_message ${MSG}
  fi
done
